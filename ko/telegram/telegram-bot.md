...
> 💡 한 라이선스 = 하나의 연결된 채팅입니다. 다른 기기/계정에서 새로 연결하면 이전 연결은 자동 해제됩니다.
...
| 📋 민팅 기록 / 🖼 포트폴리오 | 기록·보유 |
| 🛠 설정 | 봇 전용 RPC·프록시·**OpenSea API 키** |
| ℹ️ 상태 / 🌐 Language | 상태 · 한/영/중 전환 |

---

### 🔑 OpenSea API 키 (선택, WL/서명 드롭용)

**🛠 설정 → ➕ OpenSea 키**에서 키를 등록하면, 화이트리스트(서명) 드롭에서 봇이 **OpenSea 공식 Drops API**(계정당 분당 120회)로 바우처를 받습니다. 키가 없으면 익명 SWAP(프록시 IP)으로 동작하며, **퍼블릭 민트는 키가 필요 없습니다**. 키는 [docs.opensea.io](https://docs.opensea.io)에서 발급하며, 보낸 메시지는 자동으로 삭제됩니다.
...
미리 만들어 두고 ▶를 눌러두면, 봇이 **그 시각까지 기다렸다 정확히 발사**합니다(그동안 "무장됨"으로 표시).
...
> 📦 ETH(남은 돈)는 자동으로 옮겨지지 않습니다. 그건 **💸 회수**로 빼세요.
